...
@objc public class ScrollableSegmentedControl: UIControl {
    ...
    public var widthPadding : CGFloat = 60
            ...
            if oldValue != segmentStyle {
                ...
                if indexPath != nil {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.05, execute: {
                        self.collectionView?.selectItem(at: indexPath, animated: true, scrollPosition: UICollectionViewScrollPosition.right)
                    })
                }
    ...
    @objc public func insertSegment(withTitle title: String, at index: Int) {
        ...
        segment.title = title
        segment.widthPadding = self.widthPadding
        configureAttributedTitlesForSegment(segment)
    ...
    fileprivate func configureSegmentSize() {
        ...
        switch segmentStyle {
        case .imageOnLeft:
            width = longestTextWidth + BaseSegmentCollectionViewCell.imageSize + BaseSegmentCollectionViewCell.imageToTextMargin * 5
        default:
    ...
    fileprivate func reloadSegments() {
        if let collectionView_ = collectionView {
            ...
            if selectedSegmentIndex >= 0 {
                let indexPath = IndexPath(item: selectedSegmentIndex, section: 0)
                collectionView_.selectItem(at: indexPath, animated: true, scrollPosition: .right)
            }
        }
    }
    ...
    private class BaseSegmentCollectionViewCell: UICollectionViewCell {
        ...
        static let defaultFont = UIFont.systemFont(ofSize: 16.0, weight: .semibold)
        static let defaultTextColor = UIColor(red: 105.0/255.0, green: 91.0/255.0, blue: 94.0/255.0, alpha: 1.0)
        ...
        private func configureConstraints() {
            if let underline = underlineView {
                ...
                underline.heightAnchor.constraint(equalToConstant: 6.0).isActive = true
    ...
    private class TextOnlySegmentCollectionViewCell: BaseSegmentCollectionViewCell {
        ...
        override var selectedContentColor:UIColor? {
            didSet {
                titleLabel.highlightedTextColor = (selectedContentColor == nil) ? BaseSegmentCollectionViewCell.defaultTextColor : selectedContentColor!
            }
        }
            ...
            didSet {
                if isSelected {
                    if let title = super.selectedAttributedTitle {
                        ...
                        titleLabel.textColor = (selectedContentColor == nil) ? BaseSegmentCollectionViewCell.defaultTextColor : selectedContentColor!
                    }
                } else {
...
//            titleLabel.heightAnchor.constraint(equalToConstant: BaseSegmentCollectionViewCell.imageSize).isActive = true
//            titleLabel.widthAnchor.constraint(equalToConstant: BaseSegmentCollectionViewCell.imageSize).isActive = true
//            titleLabel.centerYAnchor.constraint(equalTo: contentView.centerYAnchor).isActive = true
//            titleLabel.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: BaseSegmentCollectionViewCell.textPadding).isActive = true
            imageView.centerYAnchor.constraint(equalTo: titleLabel.centerYAnchor).isActive = true
            imageView.leadingAnchor.constraint(equalTo: titleLabel.trailingAnchor, constant: BaseSegmentCollectionViewCell.textPadding).isActive = true
            contentView.trailingAnchor.constraint(greaterThanOrEqualTo: titleLabel.trailingAnchor, constant: BaseSegmentCollectionViewCell.textPadding).isActive = true
        }
    }
}
