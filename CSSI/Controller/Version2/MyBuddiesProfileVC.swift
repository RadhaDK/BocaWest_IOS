            ...
            SharedUtlity.sharedHelper().showToast(on:self.view, withMeassge: self.appDelegate.masterLabeling.nO_CONTACT_DETAILS_TOADD, withDuration: Duration.kMediumDuration)
                ...
                if availableNumbersCount > 1 {
                    ...
                    if availableNumbersCount == availableContactCount {
                        ...
                        self.nameAlreadyExist(matchedName: contactToMatch)
                    }
                    ...
                    if self.memberFirstName == matchedName?.givenName && self.memberLastName == matchedName?.familyName {
                        nameAlreadyExist(matchedName: matchedName!)
                    } else {
                        createNewContact()
                    }
                } else {
...
//        }
    ...
    // Added by Zeeshan 04262022 Multiple contacts edit issue
    func nameAlreadyExist(matchedName: CNContact) {
        
        let refreshAlert = UIAlertController(title: "\"\(matchedName.givenName) \(matchedName.familyName)\" \(self.appDelegate.masterLabeling.cONTACT_NAME_ALREADY_EXISTS ?? "")", message: self.appDelegate.masterLabeling.cONTACT_NAME_ALREADY_EXISTS_UPDATE, preferredStyle: UIAlertControllerStyle.alert)
