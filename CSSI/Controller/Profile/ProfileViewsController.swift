...
//        navigationItem.rightBarButtonItem = homeBarButton
    ...
    @objc func tapFunction(tapGestureRecognizer: UITapGestureRecognizer) {
        ...
        let alertController = UIAlertController(title:
            "Boca West", message: "Wants to use your", preferredStyle: .actionSheet)
    ...
    func getMemberInfoApi() -> Void {
        if (Network.reachability?.isReachable) == true{
            ...
            APIHandler.sharedInstance.getMemberInfoApi(paramater: dict, onSuccess: { arrgetMemberInfo in
                ...
                //Added by kiran V3.1 -- ENGAGE0012516 -- Enables user interaction only if member is 00. otherwise user is not allowed to interact with the Send magazines to and send statements to Address pickers.
                //ENGAGE0012516 -- Start
                self.txtEditSendStatementsTo.isUserInteractionEnabled = !(arrgetMemberInfo.disableSendStatementTo == 1)
                self.txtEditSendMagazineTo.isUserInteractionEnabled = !(arrgetMemberInfo.disableSendMagazineTo == 1)
                
                self.txtEditSendStatementsTo.backgroundColor = self.txtEditSendStatementsTo.isUserInteractionEnabled ? .clear : APPColor.OtherColors.disableColor
                self.txtEditSendMagazineTo.backgroundColor = self.txtEditSendMagazineTo.isUserInteractionEnabled ? .clear : APPColor.OtherColors.disableColor
                
                //Fixed the color issue with ENGAGE0012516 ticket.
                self.txtEDitCountryOther.backgroundColor = self.txtEDitCountryOther.isEnabled ? .clear : APPColor.OtherColors.disableColor
                self.txtEditCountryNuss.backgroundColor = self.txtEditCountryNuss.isEnabled ? .clear : APPColor.OtherColors.disableColor
                //ENGAGE0012516 -- End
...
//
    ...
    @IBAction func outSideUSOtherClciked(_ sender: Any) {
        ...
        //Added by kiran V3.1 -- ENGAGE0012516 -- Fixed the old color issue as part of this ticket
        //ENGAGE0012516 -- Start
        self.txtEDitCountryOther.backgroundColor = self.txtEDitCountryOther.isEnabled ? .clear : APPColor.OtherColors.disableColor
        //ENGAGE0012516 -- End
    }
